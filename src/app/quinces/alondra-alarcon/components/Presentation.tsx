...
const TimeCard = ({ value, label, scopeRef, icon }: any) => (
  ...
  >
    ...
    <div className="relative bg-gradient-to-br from-yellow-50 to-amber-50 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 shadow-lg border-2 border-yellow-300 min-w-[80px] sm:min-w-[100px] md:min-w-[120px]">
      {/* Decoración superior */}
      <div
        className={`${whisper.className} absolute -bottom-2  left-1/2 transform -translate-x-1/2 bg-yellow-500 text-white flex items-center justify-center shadow-md px-1 sm rounded text-xl`}
      >
        {label}
      </div>
...
export default function Presentation() {
  ...
  return (
    <section className="flex flex-col items-center justify-center relative overflow-hidden px-4 z-10 min-h-[100svh]">
      ...
      >
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-xl border border-yellow-200">
          <p
            className={`${quickSand.className} text-yellow-900 text-sm sm:text-base leading-relaxed text-center`}
          >
            ...
            y una vida entera por vivir. El comienzo de una nueva etapa, y por
            más que las primaveras sigan llegando, ninguna será tan especial
            como esta.
          </p>

          <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-yellow-200">
            <p
              className={`${quickSand.className} text-yellow-800 text-xs sm:text-sm md:text-base text-center`}
            >
              Acompáñanos a celebrar los{" "}
              <span className="font-bold text-yellow-700">15 años</span> de
              nuestra princesa
            </p>
            <p
              className={`${dancing.className} text-yellow-600 text-2xl sm:text-3xl md:text-4xl text-center mt-2 sm:mt-3`}
            >
